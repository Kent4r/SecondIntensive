...
# Заменяем NA на -1 (или другое уникальное значение)
df.fillna(-1, inplace=True)

# Преобразуем колонки в int64
...
def merge_columns_with_common_values(df):
    ...
    for col in df.columns:
        ...
        for next_col in df.columns:
            ...
            common_values = list(
                set(df[col].unique()).intersection(set(df[next_col].unique()))
            )
            if -1 in common_values:
                common_values.remove(-1)  # Убираем -1, если он есть

            # count jump by col number
            match = re.match(r"([a-zA-Z]+)(\d{1,4})", col)
            match1 = re.match(r"([a-zA-Z]+)(\d{1,4})", next_col)

            # Проверяем, что match и match1 не None
            if match is None or match1 is None:
                print(f"Skipping columns {col} and {next_col} due to invalid format.")
                continue
            ...
            if (
                len(common_values) >= 1
            ):  # Объединяем, если есть хотя бы одно общее значение
                if calc_jump():
                    temp_col = next_col
                    break
                count += 1
                ...
                if len(common_values) >= 1:
                    ...
                    unique_columns.append(next_col)  # Помечаем колонку как обработанную
                    print(
                        f"Merged {col} and {next_col}: common_values = {common_values}"
                    )
                else:
                    break
    ...
    for group_name, group_columns in column_groups.items():
        if len(group_columns) > 1:  # Если в группе больше одной колонки
            df[group_name] = (
                df[group_columns].bfill(axis=1).iloc[:, 0]
            )  # Объединяем колонки
